...
public class ZimbraPort {
    // private static final String A_TAGS = "tags";
    ...
    public ZimbraPort(URL _endpoint) {
        endpoint = _endpoint;
        servletUploadURL = endpoint.getProtocol() + "://" + endpoint.getAuthority()
                + "/service/upload?fmt=raw";
        servletDownloadURL = endpoint.getProtocol() + "://" + endpoint.getAuthority()
                + "/service/home/~/?";
        connOpen = false;
    ...
    public ArrayList<Contact> requestAllContact(boolean trashInclude) throws IOException,
     ...
     *             Invalid soap document response was received
     */
    public Contact requestContactById(String contactId, boolean syncPhoto) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }
        ...
        if (imageAttrib != null && syncPhoto) {
            // http://yourserver.com/service/home/~/?id={contact-id}&part=1
            String part = imageAttrib.attributeValue("part");
            String url = servletDownloadURL + "id=" + contactId + "&part=" + part;
            byte[] image = ConnectionUtils.download(url, ccontext);
            Photo photo = new Photo();
    ...
    public Element requestGALContactIds() throws IOException, SoapRequestException {
        ...
        Document request = createZimbraCall(ZConst.AUTO_COMPLETE_GAL_REQUEST,
                                            ZConst.URN_ZIMBRA_ACCOUNT);
        Element contactsRequest = (Element) soapHelper.getBody(request).elements().get(0);
        {
        ...
        Document response = sendRequest(request/* .asXML().replace("{1}", "") */);

        Element contactsResponse = soapHelper.getBody(response)
                                             .element(ZConst.AUTO_COMPLETE_GAL_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
     ...
     */
    @SuppressWarnings("unchecked")
    public void requestRemoveAllItems(Element itemsResponse, String attrIdName) throws IOException,
            SoapRequestException {
        ...
        for (Iterator<Element> iterator = itemsResponse.elementIterator(); iterator.hasNext();) {
            ...
            if (attr == null)
                throw new SoapRequestException("Invalid attribute in element");
    ...
    public Element requestCreateContact(Contact c) throws IOException, SoapRequestException {
        ...
        Element contactsRequest = (Element) soapHelper.getBody(request).elements().get(0);
        {
            Element contact = ContactUtils.asElement(c,
                                                     documentFactory,
        ...
        Document response = sendRequest(request);
        Element contactsResponse = soapHelper.getBody(response)
                                             .element(ZConst.CREATE_CONTACT_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
        return contactsResponse;
    }
    ...
    private void addPhotoAttributeToContact(Contact c, Element contact, boolean u) {
        ...
        if (photo != null && photo.getImage() != null && photo.getImage().length > 0) {
            try {
                String aid = ConnectionUtils.upload("image",
                                                    photo.getImage(),
                                                    ccontext,
                                                    servletUploadURL);
                Element attr = documentFactory.createElement(ZConst.A_ATTRIBUTE);
     ...
     *             Invalid soap document response was received
     */
    public Element requestModifyContact(Contact c, String oldFolderId, Object oldTags)
            throws IOException, SoapRequestException {
        if (!init()) {
            return null;
        }
        String folderName = c.getFolder();
        ...
        Element contactsRequest = body.addElement(ZConst.MODIFY_CONTACT_REQUEST,
                                                  ZConst.URN_ZIMBRA_MAIL);
        {
            Element contact = ContactUtils.asElement(c,
                                                     documentFactory,
     ...
     *             Invalid soap document response was received
     */
    public Element requestDeleteContact(String contactId, boolean soft) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }
        ...
        Document response = sendRequest(request);
        Element contactsResponse = soapHelper.getBody(response)
                                             .element(ZConst.CONTACT_ACTION_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
        return contactsResponse;
    }
     ...
     *             Invalid soap document response was received
     */
    public Element requestDeleteItem(String id, boolean soft) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }
        ...
        Document response = sendRequest(request);
        Element contactsResponse = soapHelper.getBody(response)
                                             .element(ZConst.ITEM_ACTION_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
        return contactsResponse;
    }
    ...
    private Document sendRequest(Document request) throws IOException, SoapRequestException {
        try {
            ...
            // dump last request message, if haven't done before
            if (!writer.isVerboseEnable())
                writer.write(request);
            throw e;
        }
    }
     ...
     */
    public ArrayList<Calendar> requestAllCalendars(ArrayList<String> ids, boolean task, TimeZone phoneTimeZone)
            throws IOException, SoapRequestException, ConversionException {
        if (!init()) {
            return null;
        }
        ...
        for (Element calResponse : calResponses) {
            ...
            try {
                ...
                writer.write(item);
                logger.error("", e);
                // if occur exception we should throw exception after all
                // convertion exceptions
                isExceptionOccur = true;
            }
        }
        processFault(batch, request, response);
        if (isExceptionOccur)
            throw new ConversionException("Error occur while converting calendars");
        return calendars;
    }
     ...
     *             Invalid soap document response was received
     */
    public Calendar requestCalendarById(String calendarId, boolean task, TimeZone phoneTimeZone) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }

        Document request = createZimbraCall(task ? ZConst.GET_TASK_REQUEST
                : ZConst.GET_APPOINTMENT_REQUEST, ZConst.URN_ZIMBRA_MAIL);
        Element calendarRequest = (Element) soapHelper.getBody(request).elements().get(0);
        {
        ...
        Element body = soapHelper.getBody(response);
        Element callResponse = body.element(task ? ZConst.GET_TASK_RESPONSE
                : ZConst.GET_APPOINTMENT_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
    ...
    public Element requestCreateCalendar(Calendar c) throws IOException, SoapRequestException {
        ...
        Element calendarRequest = batch.addElement(task ? ZConst.CREATE_TASK_REQUEST
                : ZConst.CREATE_APPOINTMENT_REQUEST, ZConst.URN_ZIMBRA_MAIL);
        {
            Element calendar = CalendarUtils.getInstance().asElement(c,
                                                                     documentFactory,
                                                                     ZConst.URN_ZIMBRA_MAIL,
                                                                     false);
        ...
        // need invoke this methods before sendRequest, else will be duplicate
        // init call
        ...
        body = soapHelper.getBody(response);
        batch = body.element(ZConst.BATCH_RESPONSE);
        Element calendarResponse = batch.element(task ? ZConst.CREATE_TASK_RESPONSE
                : ZConst.CREATE_APPOINTMENT_RESPONSE);
        processFault(batch, request, response);
        ccontext.processContext(soapHelper.getContext(response));
        if (!init()) {
            return null;
        ...
        if (tagsIds != null && !tagsIds.equals("") && !tagsIds.equals("null")) {
            ...
            boolean empty = attachTagItemRequest(batch,
                                                 calendarResponse.attributeValue("invId"),
                                                 tagsIds);
            if (!empty) {
                response = sendRequest(request);
                body = soapHelper.getBody(response);
                ...
                for (Element resp : tags) {
                    ...
                    tagHolder.put(tag.attributeValue(ZConst.A_ID),
                                  tag.attributeValue(ZConst.A_TAGNAME));
                }
                ccontext.processContext(soapHelper.getContext(response));
            }
        }
     ...
     *             Invalid soap document response was received
     */
    private String createFolderRequest(String folderName, String folderType) throws IOException,
            SoapRequestException {
        ...
        if (folderId == null) {
            Document request = createZimbraDoc();
            Element body = soapHelper.getBody(request);
            Element createFolderRequest = body.addElement(ZConst.CREATE_FOLDER_REQUEST,
                                                          ZConst.URN_ZIMBRA_MAIL);
            ...
            // we have to create folder before create item
            Document response = sendRequest(request);
            ccontext.processContext(soapHelper.getContext(response));
            Element folderResponse = soapHelper.getBody(response)
                                               .element(ZConst.CREATE_FOLDER_RESPONSE);
    ...
    private String createTagsRequest(String tagsStr) throws IOException, SoapRequestException {
        ...
        for (String tagName : tags) {
            ...
            Element createTagRequest = batch.addElement(ZConst.CREATE_TAG_REQUEST,
                                                        ZConst.URN_ZIMBRA_MAIL);
            Element tagEl = createTagRequest.addElement(ZConst.E_TAG);
            tagEl.addAttribute(ZConst.A_TAGNAME, tagName);
        }
        if (batch.hasContent()) {
            // we have to create tags before create item
            Document response = sendRequest(request);
            ccontext.processContext(soapHelper.getContext(response));
            batch = soapHelper.getBody(response).element(ZConst.BATCH_RESPONSE);
            processFault(batch, request, response);
    ...
    public void attachMoveItemRequest(Element body, String itemId, String folderId) {
        Element createFolderRequest = body.addElement(ZConst.ITEM_ACTION_REQUEST,
                                                      ZConst.URN_ZIMBRA_MAIL);
    ...
    public void attachMoveContactRequest(Element body, String itemId, String folderId) {
        Element createFolderRequest = body.addElement(ZConst.CONTACT_ACTION_REQUEST,
                                                      ZConst.URN_ZIMBRA_MAIL);
        Element folder = createFolderRequest.addElement(ZConst.E_ACTION);
        folder.addAttribute(ZConst.A_ID, itemId);
        folder.addAttribute(ZConst.A_FOLDER, folderId);
     ...
     *             Invalid soap document response was received
     */
    public Element requestModifyCalendar(String uid, Calendar c, String oldFolderId, String oldTags)
            throws IOException, SoapRequestException {
        if (!init()) {
            return null;
        }
        ...
        Document request = createZimbraDoc();
        Element body = soapHelper.getBody(request);
        Element calendarRequest = body.addElement(task ? ZConst.MODIFY_TASK_REQUEST
                : ZConst.MODIFY_APPOINTMENT_REQUEST, ZConst.URN_ZIMBRA_MAIL);
        {
            Element calendar = CalendarUtils.getInstance().asElement(c,
                                                                     documentFactory,
                                                                     ZConst.URN_ZIMBRA_MAIL,
    ...
    public Element requestCreateNote(Note n) throws IOException, SoapRequestException {
        ...
        Document response = sendRequest(request);
        Element contactsResponse = soapHelper.getBody(response)
                                             .element(ZConst.CREATE_NOTE_RESPONSE);
        ccontext.processContext(soapHelper.getContext(response));
        return contactsResponse;
    }
    ...
    public static void main(String[] args) throws ParserConfigurationException, IOException,
            SoapRequestException {
        // Don't publish this keystore!
        System.setProperty("javax.net.ssl.trustStore",
                           "E:/Work/_Project/Funambol/funambol-connector-zimbra/ssl-truststore/keystore");
    ...
    public Element requestModifyNote(String key, Note n, String oldFolderId) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }
        ...
        // if (!oldFolderId.equals(folderId)) {
        // batchRequest = true;
        // Element batch = convertToBatch(body);
        // attachMoveContactRequest(batch, key, folderId);
        // }
    ...
    public Element requestIcal(String calendarId) throws IOException,
            SoapRequestException {
        if (!init()) {
            return null;
        }
